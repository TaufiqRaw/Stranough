...
import { GuitarBody, GuitarBodyTextureKeyType, ElectricModel, GuitarModelBodyKeyType, SpawnPointType } from "../types";
...
export function createModel(
  ...
  const [selectedBody, setSelectedBody] = createSignal<GuitarModelBodyKeyType | undefined>(
    props?.selectedBody ?? (props?.boltOnBody ? "boltOnBody" : props?.neckThroughBody ? "neckThroughBody" : props?.setInBody ? "setInBody" : undefined));
    ...
    spawnPoints: {
      ...
      getSelectedSignal: () => {
        switch (obj.spawnPoints.selected.get()) {
          ...
          case GuitarBodySPEnum.fingerboardBackEnd :
            return {
      ...
      asArray: () => [
        ...R.pipe(
          obj.spawnPoints,
          R.pick(["bridge", "fingerboard", 'fingerboardBackEnd']),
          R.values,
