...
export class ChatbotService {
  ...
  async ask(id : string,input : string){
    ...
    if(typeof response === 'string'){
      userContext.chats.push({role : 'assistant', content : response});
      return response;
    }
    else{
      // for some reason, this code will make LLM generate json in a string, so i comment this code
      // userContext.chats.push({role : 'assistant', content : JSON.stringify(response)});
...
async function askRequest(context : IUserChatContext, validValue ?: {[ k in keyof GuitarBuilder.SelectedItem] ?: any[]}, lastSelectedComponents ?: GuitarBuilder.SelectedItem){
  const res = await DI.openAi.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "You are a virtual assistant at Stranough guitar technology that will help user to build a custom guitar, make sure your answer are concise and clear, no unecessary long text, here are some rules : if the user ask for recommendation or ask you to select a component, use the select_component tool, Don't make assumptions about what values to plug into functions. Ask for clarification if a user request is ambiguous."
      },
      ...context.chats
    ],
    tools: [
  ...
  function: {
    ...
    parameters: {
      ...
      properties: {
        ...
        guitarModel : {
          type : 'string',
          enum : loadedValidValue?.guitarModel ?? ['get-valid-value']
        },
        constructionMethod : {
          type : 'string',
          enum : GuitarModel.bodyKeys
        },
        topContour : {
          type : 'string',
          enum : GuitarBody.topContourKeys
        },
        backContour : {
          type : 'string',
          enum : GuitarBody.backContourKeys
        },
        bodyCoreWood : {
          type : 'string',
          enum : GuitarBuilder.bodyCoreWoods.map(v=>v.name)
        },
        bodyTopWood : {
          type : 'string',
          enum : GuitarBuilder.bodyTopWoods.map(v=>v.name)
        },
        bodyColorType : {
          type : 'string',
          enum : [...GuitarBuilder.bodyColorType.map(v=>v.name), 'transparent']
        },
        bodyColor : {
          type : 'string',
          enum : loadedValidValue?.bodyColor ?? ['get-valid-value']
        },
        neckWood : {
          type : 'string',
        ...
        headstock : {
          type : 'string',
          enum : loadedValidValue?.headstock ?? ['get-valid-value']
        },
        peg : {
          type : 'string',
          enum : loadedValidValue?.peg ?? ['get-valid-value']
        },
        bridge : {type : 'string',
          enum : loadedValidValue?.bridge ?? ['get-valid-value']
        },
        jack : {
          type : 'string',
          enum : loadedValidValue?.jack ?? ['get-valid-value']
        },
        knob : {
          type : 'string',
          enum : loadedValidValue?.knob ?? ['get-valid-value']
        },
