...
export function entityGetMiddleware<T extends BaseEntity>(repository : ()=>EntityRepository<T>, _options? : FindOneOptions<T> & {itemCallback ?: (item : T)=>Promise<T>}){
  return asyncMiddleware(async(req,res)=>{
    const id = parseInt(req.params.id);
    
    const {itemCallback, ...options} = _options ?? {};
    
    const item = await findOneEntity(repository(), id, options as FindOneOptions<T>);
