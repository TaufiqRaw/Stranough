...
import Dragable, { DragableController } from "~/commons/components/dragable";
import { useGuitarBuilderContext } from "../../guitar-builder";
import Lottie from "lottie-web";

export default function GuiSubMenuSideMobile(
  props : {
  ...
  const {get : isSwiped, swiped} = useGuitarBuilderContext()!.isBottomSideMenuSwiped;
  ...
  onMount(()=>{
    ...
    if(swipeAnimContainerRef){
      var swipeAnimation = Lottie.loadAnimation({
        ...
        path: '/assets/swipe-up.json', // Required
  ...
  return <div
      class={'bg-gray-700 text-white h-44 transform shrink-0 touch-none overflow-hidden relative transition-transform '}
      style={{
    ...
    > 
      ...
      >
        <For each={props.items}>
          {(item, i) => <div 
            class={'item ' 
              + (props.selectedSubmenuIndex() == i() ? 'bg-primary bg-opacity-40 ' : '')}
          >
